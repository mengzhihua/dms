...
if [[ "$ok" != "1" ]]; then
  echo "SMOKE FAIL dms: app did not start on :$PORT"
  tail -n 80 "$DIR/smoke.log" || true
  exit 1
fi
html="$(curl -sS "http://127.0.0.1:${PORT}/")"
echo "$html" | grep -qiE '<html|<div id=.app' || { echo "SMOKE FAIL dms: / is not HTML"; exit 1; }
spa="$(curl -sS -o /tmp/dms-spa.body -w "%{http_code}" "http://127.0.0.1:${PORT}/dashboard")"
test "$spa" = "200"
